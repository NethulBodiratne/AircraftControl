...
// We need to mock the Arduino functions and a few global variables
// to allow the code to compile and run on a standard C++ environment.

#define HIGH 1
#define LOW 0
#define INPUT 0
#define OUTPUT 1
#define BUILTIN_CS 10

unsigned long mock_millis_time = 0;
int mock_pin_states[14] = {HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH, HIGH};
bool error_led_state = LOW;
bool sd_card_available = true;

// Mock classes for libraries
class MockWire {
public:
    void begin() {}
};
...
class MockSD {
public:
    bool begin(int pin) { return sd_card_available; }
    class File {
    public:
    ...
    File open(const std::string& filename, int mode) { return File(); }
...
void digitalWrite(int pin, int state) {
    if (pin == ERROR_LED_PIN) {
        error_led_state = state;
    }
}

// ---------------------------- MOCK SENSOR & CONTROL FUNCTIONS ---------------------------- //
// These mocks allow us to control the "sensor data" for our tests.
float mock_accel_speed = 0.0;
float mock_throttle = 0.0;
float mock_elevator = 0.0;
float mock_aileron_left = 0.0;
float mock_aileron_right = 0.0;
float mock_rudder = 0.0;
float mock_altitude = 0.0;
float mock_roll = 0.0;

bool isPlaneMoving() {
    return mock_accel_speed > 0.1;
}
void readSensors() {
    // In our test, we'll manually set the mock_accel_speed
}
float getAltitude() { return mock_altitude; }
float getSpeed() { return mock_accel_speed; }
float getThrottle() { return mock_throttle; }
float getElevatorAngle() { return mock_elevator; }
float getAileronLeftAngle() { return mock_aileron_left; }
float getAileronRightAngle() { return mock_aileron_right; }
float getRudderAngle() { return mock_rudder; }
float getRollAngle() { return mock_roll; }
bool resetSensors() { return true; }

void setThrottle(float val) { mock_throttle = val; }
void setElevator(float val) { mock_elevator = val; }
void setAileronLeft(float val) { mock_aileron_left = val; }
void setAileronRight(float val) { mock_aileron_right = val; }
void setRudder(float val) { mock_rudder = val; }
...
// Helper function to simulate time progression and loop calls
void run_loop_for_duration(unsigned long duration_ms) {
    unsigned long start_time = mock_millis_time;
    while (mock_millis_time - start_time < duration_ms) {
        loop();
        mock_millis_time += 100; // Simulate 100ms passing
    }
}

// ---------------------------- CODE UNDER TEST ---------------------------- //
// The user's code is included here
#include <Wire.h>
#include <SPI.h>
#include <SD.h>
...
float getAltitude() {
  return mock_altitude; // Returns the value from the distance sensor that is pointed at the ground to get altitude
}

// ----------------------------  CURRENT SPEED  ---------------------------- //
/* Get the current speed. */
float getSpeed() {
  return mock_accel_speed; // Return the speed
}

// ----------------------------  CURRENT THROTTLE  ---------------------------- //
/* Gets the current throttle value. */
float getThrottle() {
  return mock_throttle; // Return the throttle power
}

// ----------------------------  CURRENT ROLL ANGLE  ---------------------------- //
/* Get the current roll angle. */
float getRollAngle() {
  return mock_roll; // Return the roll angle
}

// ----------------------------  CURRENT ELEVATOR ANGLE  ---------------------------- //
/* Gets the current elevator angle. */
float getElevatorAngle() {
  return mock_elevator; // Return the elevator angle
}

// ----------------------------  CURRENT LEFT AILERON  ---------------------------- //
/* Gets the current left aileron angle. */
float getAileronLeftAngle() {
  return mock_aileron_left;
}

// ----------------------------  CURRENT RIGHT AILERON  ---------------------------- //
/* Gets the current right aileron angle. */
float getAileronRightAngle() {
  return mock_aileron_right;
}
...
void setThrottle(float throttleValue) {
  currentThrottle = std::max(MIN_THROTTLE, std::min(MAX_THROTTLE, throttleValue));
  mock_throttle = currentThrottle;
...
void setElevator(float elevatorAngle) {
  currentElevator = std::max(MIN_ELEVATOR, std::min(MAX_ELEVATOR, elevatorAngle));
  mock_elevator = currentElevator;
...
void setAileronLeft(float leftAileronAngle) {
  currentAileronLeft = std::max(MIN_AILERON, std::min(MAX_AILERON, leftAileronAngle));
  mock_aileron_left = currentAileronLeft;
  // return ;
}
...
void setAileronRight(float rightAileronAngle) {
  currentAileronRight = std::max(MIN_AILERON, std::min(MAX_AILERON, rightAileronAngle));
  mock_aileron_right = currentAileronRight;
  // return ;
}
...
void setRudder(float rudderAngle) {
  currentRudder = std::max(MIN_RUDDER, std::min(MAX_RUDDER, rudderAngle));
  mock_rudder = currentRudder;
  // return ;
}
...
// ---------------------------- TEST CASES ---------------------------- //
TEST_SUITE("Flight Control Mode Tests") {

    TEST_CASE("MODE_TAXI - Plane is Stopped") {
        // Setup initial state for a clean test
        mock_millis_time = 0;
        mock_pin_states[MODE_0] = LOW; // Represents binary 0011 which is 3,
        mock_pin_states[MODE_1] = LOW;  // corresponding to MODE_TAXI in your code.
        mock_pin_states[MODE_2] = HIGH;
        mock_pin_states[MODE_3] = HIGH;
        mock_pin_states[MODE_4] = HIGH;
        mock_accel_speed = 0.05; // Simulate a stationary plane (speed < 0.1)

        // Run the setup function to initialize the mode
        setup();
        CHECK(currentMode == MODE_TAXI);
        CHECK(lastMode == MODE_TAXI);

        // Run the loop for less than the TAXI_DURATION
        run_loop_for_duration(TAXI_DURATION - 1000);

        // Assertions for when the plane is taxiing but not yet stopped
        CHECK(mock_throttle > 0.0);
        CHECK(mock_rudder == 0.0);
        CHECK(error_led_state == LOW);
        CHECK(isPlaneMoving() == false);
        
        // Assert that after the taxi duration, the plane stops
        mock_millis_time = TAXI_DURATION;
        loop();
        CHECK(mock_throttle == 0.0);
        CHECK(mock_rudder == 0.0);
        CHECK(error_led_state == LOW);
    ...
    TEST_CASE("MODE_TAXI - Plane is Moving") {
        // Setup initial state for a clean test
        mock_millis_time = 0;
        mock_pin_states[MODE_0] = LOW;
        mock_pin_states[MODE_1] = LOW;
        mock_pin_states[MODE_2] = HIGH;
        mock_pin_states[MODE_3] = HIGH;
        mock_pin_states[MODE_4] = HIGH;
        mock_accel_speed = 0.5; // Simulate a moving plane (speed > 0.1)
        ...
        // Assertions for when the plane is taxiing and moving
        CHECK(mock_throttle > 0.0);
        CHECK(mock_rudder == 0.0);
        CHECK(error_led_state == HIGH);
        CHECK(isPlaneMoving() == true);

        // Assert that after the taxi duration, the plane stops but an error is logged
        mock_millis_time = TAXI_DURATION;
        loop();
        CHECK(mock_throttle == 0.0);
        CHECK(mock_rudder == 0.0);
